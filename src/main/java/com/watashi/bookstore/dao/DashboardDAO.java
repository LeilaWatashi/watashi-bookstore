...
import com.watashi.bookstore.domain.grafico.Serie;
...
public class DashboardDAO implements IDAO {
    ...
    public List<EntidadeDominio> consultar(EntidadeDominio entidade) {
        ...
        if(dashboard.getTipoGrafico().trim().equalsIgnoreCase("ANTIGO")) {
        ...
        if(dashboard.getTipoGrafico().trim().equalsIgnoreCase("VENDAPORCATEGORIA")){

            dashboard.setTitle("Vendas Mensais por Categoria");

            List<Pedido> pedidosFiltrados = pedidoRepository.getDataToSellDashboard(dashboard.getDataInicio(), dashboard.getDataFim());

            List<CategoriaLivro> categorias = (List<CategoriaLivro>) categoriaLivroRepository.findAll();
            ...
            for (CategoriaLivro categoria : categorias) {
            ...
            for (Pedido pedidoFiltrado : pedidosFiltrados) {
                ...
                for (Item item : pedidoFiltrado.getItemList()) {
                    Integer quantidadeAtual = mapaCategoriaContadorQuantidade.get(item.getLivro().getCategoriaLivro().getNome());
                    mapaCategoriaContadorQuantidade.put(item.getLivro().getCategoriaLivro().getNome(), ++quantidadeAtual);
                }
                ultimaDataAnalisada = dataAtualParaAnalise;
            }
